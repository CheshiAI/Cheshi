 ...
 *
 * The CLI itself is exercised end-to-end against the Bun source CLI so the JSON
...
describe('index completeness marker (index_state)', () => {
  ...
  it('a run killed mid-index leaves state=indexing, and status --json surfaces it', async () => {
    ...
    const db = new Database(path.join(tempDir, '.codegraph', 'codegraph.db'), { strict: true });
    db.prepare(
