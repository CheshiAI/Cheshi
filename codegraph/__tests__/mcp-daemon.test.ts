 ...
 *
 * These tests intentionally spawn real `bun src/bin/codegraph.ts` processes
...
function spawnServer(cwd: string, env: NodeJS.ProcessEnv = {}): SpawnedServer {
  ...
  // unhandled error that crashes the test runner.
...
describe('Shared MCP daemon (issue #411)', () => {
  ...
  afterEach(async () => {
    ...
    // lockfile, and we must never SIGKILL the test runner.
  ...
  it('daemon idle-times-out after the last client disconnects', async () => {
    ...
    const daemonExited = await waitProcessExit(daemonPid, 10000);
    if (!daemonExited) {
      throw new Error(
        `daemon did not exit after its last client disconnected; ` +
