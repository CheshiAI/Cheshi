 ...
 * Under parallel test execution the OS watch subsystems (FSEvents / inotify) serve
...
describe('FileWatcher', () => {
  ...
  describe('start/stop lifecycle', () => {
    it('should start and stop without errors', () => {
      const syncFn = mock().mockResolvedValue({ filesChanged: 0, durationMs: 0 });
  ...
  describe('watch-resource exhaustion (#876)', () => {
    ...
    it('fails to start and degrades when fs.watch setup exhausts watch resources', () => {
      ...
      const watcher = new FileWatcher(
        dir,
        mock().mockResolvedValue({ filesChanged: 0, durationMs: 0 }),
    ...
    it('reports isDegraded false / null reason while healthy', () => {
      const watcher = newWatcher(mock().mockResolvedValue({ filesChanged: 0, durationMs: 0 }));
      watcher.start();
  ...
  describe('lock contention degradation (#876)', () => {
    it('disables auto-sync after prolonged lock contention, with bounded retries', async () => {
      const syncFn = mock().mockRejectedValue(new LockUnavailableError());
      const onSyncComplete = mock();
  ...
  describe('persistent sync-failure degradation (#1127)', () => {
    it('disables auto-sync after a persistent non-lock sync failure, with bounded retries', async () => {
      ...
      const syncFn = mock().mockRejectedValue(new Error('extractor crashed on src/bad.ts'));
      const onSyncComplete = mock();
      const onSyncError = mock();
      const onDegraded = mock();
      const warnSpy = spyOn(console, 'warn').mockImplementation(() => {});
  ...
  describe('debounced sync', () => {
    it('should trigger sync after file change', async () => {
      const syncFn = mock().mockResolvedValue({ filesChanged: 1, durationMs: 10 });
  ...
  describe('pending file tracking (#403)', () => {
    ...
    it('should retain an event that arrives mid-sync in the same millisecond', async () => {
      const firstSyncStarted = createDeferred();
      const releaseFirstSync = createDeferred();
      let syncCalls = 0;
      const syncFn = mock(async () => {
        syncCalls++;
        if (syncCalls === 1) {
        ...
        return { filesChanged: 1, durationMs: 10 };
      });
      const onSyncComplete = mock();
      const watcher = newWatcher(syncFn, { debounceMs: 100, onSyncComplete });
      watcher.start();
      await watcher.waitUntilReady();

      const nowSpy = spyOn(Date, 'now').mockReturnValue(123456);
      let nowIsMocked = true;
      try {
        __emitWatchEventForTests(testDir, 'src/same-ms.ts');
        await firstSyncStarted.promise;
        expect(watcher.getPendingFiles().find((p) => p.path === 'src/same-ms.ts')?.indexing).toBe(true);

        // This event has the same timestamp as the event already being synced,
        // but a newer sequence. The first sync must not clear it.
        __emitWatchEventForTests(testDir, 'src/same-ms.ts');
        expect(watcher.getPendingFiles().find((p) => p.path === 'src/same-ms.ts')?.indexing).toBe(false);

        nowSpy.mockRestore();
        nowIsMocked = false;
        releaseFirstSync.resolve();
        await waitFor(() => onSyncComplete.mock.calls.length >= 1);
        expect(watcher.getPendingFiles().some((p) => p.path === 'src/same-ms.ts')).toBe(true);

        await waitFor(() => syncFn.mock.calls.length >= 2);
        await waitFor(() => !watcher.getPendingFiles().some((p) => p.path === 'src/same-ms.ts'));
      } finally {
        if (nowIsMocked) nowSpy.mockRestore();
        releaseFirstSync.resolve();
        watcher.stop();
      }
    });
    ...
    it('should retain pending files and retry when syncFn throws LockUnavailableError (#449)', async () => {
      // CodeGraph.sync() reports cross-process lock contention as a typed
      // error so the watcher's retry path picks it up instead of falsely
      // clearing pendingFiles. This test exercises the contract directly.
      const syncFn = mock()
        .mockRejectedValueOnce(new LockUnavailableError())
  ...
  describe('callbacks', () => {
    it('should call onSyncComplete after successful sync', async () => {
      const syncFn = mock().mockResolvedValue({ filesChanged: 2, durationMs: 50 });
      const onSyncComplete = mock();
      const watcher = newWatcher(syncFn, {
        debounceMs: 200,
        onSyncComplete,
    ...
    it('should call onSyncError when sync throws', async () => {
      const syncFn = mock().mockRejectedValue(new Error('sync failed'));
      const onSyncError = mock();
      const watcher = newWatcher(syncFn, {
        debounceMs: 200,
        onSyncError,
  ...
  describe('CodeGraph integration', () => {
    ...
    it('should auto-sync when files change while watching (real fs.watch end-to-end)', async () => {
      ...
      await waitFor(() => {
        ...
        return syncCompleted && stats.nodeCount > initialNodes;
      }, 8000);
