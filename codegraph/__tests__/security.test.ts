...
describe('FileLock', () => {
  ...
  it('should never expire a lock whose PID is still alive based on age alone', () => {
    const owner = new FileLock(lockPath);
    const contender = new FileLock(lockPath);
    owner.acquire();

    // Long-running index/maintenance jobs can legitimately exceed two minutes.
  ...
  it('should release lock even if async function throws', async () => {
    ...
    await expectRejection(
      ...
      (error) => {
        if (!(error instanceof Error)) throw new Error('Expected withLockAsync to reject with an Error');
        expect(error.message).toContain('async error');
      },
    );
...
describe('validateProjectPath — sensitive directory blocking', () => {
  ...
  it.if(process.platform !== 'win32')('blocks POSIX system directories (exact match)', () => {
