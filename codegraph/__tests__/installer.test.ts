...
describe('Installer Config Writer', () => {
  ...
  describe('readJsonFile error handling', () => {
    ...
    it('should refuse to overwrite corrupted JSON when backup creation fails', () => {
      const mcpJson = path.join(tempDir, '.mcp.json');
      const corrupted = '{ this is not valid json !!!';
      fs.writeFileSync(mcpJson, corrupted);
      // copyFileSync cannot replace a directory, deterministically forcing the
      // backup step to fail on every supported platform.
      fs.mkdirSync(mcpJson + '.backup');
      const warnSpy = spyOn(console, 'warn').mockImplementation(() => {});

      try {
        expect(() => writeMcpConfig('local')).toThrow(/refusing to overwrite/);
        expect(fs.readFileSync(mcpJson, 'utf-8')).toBe(corrupted);
        expect(fs.statSync(mcpJson + '.backup').isDirectory()).toBe(true);
      } finally {
        warnSpy.mockRestore();
      }
    });
