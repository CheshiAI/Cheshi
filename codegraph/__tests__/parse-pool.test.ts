...
import type { Language, ExtractionResult } from '../src';
...
async function expectRejection(
  ...
  try {
    ...
    if (!(error instanceof Error)) throw new Error('Expected operation to reject with an Error');
...
describe('ParseWorkerPool', () => {
  ...
  it('rejects a parse whose worker crashes (retry-pass-recognisable message) and keeps serving', async () => {
    ...
    await expectRejection(pool.requestParse(task('poison.ts')), /Worker exited/);
  ...
  it('times out a hung parse (at the hard-kill backstop) and stays usable', async () => {
    ...
    await expectRejection(pool.requestParse(task('hang.ts')), /timed out after 90ms/i);
  ...
  it('destroy() rejects in-flight and subsequent parses', async () => {
    ...
    await sleep(10);
    await pool.destroy();
    await expectRejection(p, /destroyed/);
    await expectRejection(pool.requestParse(task('y.ts')), /destroyed/);
  });
});
