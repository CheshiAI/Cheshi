...
describe('getDaemonSocketCandidates (#997)', () => {
  it.if(POSIX)('returns [in-project, tmpdir] for a normal short path', () => {
  ...
  it.if(POSIX)('drops straight to [tmpdir] when the in-project path is too long', () => {
  ...
  it.if(POSIX)('is deterministic and project-scoped: same root → same tmpdir fallback', () => {
  ...
  it.if(!POSIX)('returns a single named pipe on Windows', () => {
...
describe('bindFirstUsableSocket (#997)', () => {
  ...
  it('does NOT relocate on EADDRINUSE — it propagates even with a fallback present', async () => {
    ...
    await expectRejection(
      ...
      (error) => expect(error).toMatchObject({ code: 'EADDRINUSE' }),
    );
  ...
  it('propagates a capability error on the LAST candidate (nowhere left to go)', async () => {
    ...
    await expectRejection(
      ...
      (error) => expect(error).toMatchObject({ code: 'ENOTSUP' }),
    );
  });
...
describe('lock acquisition without hard links (#997)', () => {
  ...
  it.if(POSIX)('tryAcquireDaemonLock still acquires on a normal FS, and a second caller is told it is taken', () => {
  ...
  it.if(POSIX)('acquireLockViaExclusiveOpen creates the pidfile with a complete, parseable record', () => {
  ...
  it.if(POSIX)('acquireLockViaExclusiveOpen is exclusive: the second caller loses (EEXIST → false)', () => {
