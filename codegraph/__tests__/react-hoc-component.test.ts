...
describe('React HOC-wrapped component recognition (#841)', () => {
  ...
  async function index() {
    ...
    await cg.indexAll();
    return getCodeGraphState(cg).db.getDb();
  }
  ...
  it('classifies forwardRef, memo, and styled consts as component nodes', async () => {
    fs.writeFileSync(
      path.join(dir, 'ui.tsx'),
      /* language=TEXT */ `import * as React from 'react';
...
`
  ...
  it('emits jsx-render edges so callers and impact radius resolve a forwardRef component', async () => {
    fs.writeFileSync(
