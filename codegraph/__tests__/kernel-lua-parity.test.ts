 ...
 *
 * The full-repo sweeps use `bun run kernel:parity` (kong/lazy.nvim/
 * lua-resty-core + lune/Fusion for the §5 gate); this suite keeps the
 * invariant alive in `bun test`. Skips when no kernel binary is staged;
...
describe.skipIf(!kernelBuilt)('kernel Lua/Luau extraction parity', () => {
  ...
  it('cross-dialect syntax defers to the wasm extractor', () => {
    ...
    expect(
      tryKernelExtract('src/defaultparam.luau', /* language=TEXT */ 'type S<T = U> = {}\n', 'luau')
    ).toBeNull();
    process.env.CODEGRAPH_KERNEL = '0';
