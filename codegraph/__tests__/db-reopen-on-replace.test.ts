...
const posixOnly = it.if(process.platform !== 'win32');
const windowsOnly = it.if(process.platform === 'win32');
...
describe('CodeGraph.reopenIfReplaced (issue #925)', () => {
  ...
  posixOnly('heals a held connection after the index is removed and recreated at the same path', async () => {
    ...
    // Do not query the stale handle before healing. better-sqlite3 continued
    // serving the unlinked inode here, but bun:sqlite fails closed with
    // SQLITE_IOERR_VNODE once the removed WAL directory is gone. The inode
    // replacement check is the portable signal this API is designed around.
