...
import { getKernel } from '../src/extraction/kernel';
...
describe.if(nativeAvailable)('cFnPtr sweep: native vs JS differential', () => {
  let dir: string;
  ...
  const indexAndCollect = async (): Promise<{ edges: EdgeRow[]; nodes: number }> => {
    ...
    const edges: EdgeRow[] = db
      .prepare(
        `SELECT s.name AS src, t.name AS tgt, json_extract(e.metadata,'$.via') AS via, e.line AS line
      ...
      .all();
    const nodes = db.prepare('SELECT count(*) AS c FROM nodes').get().c as number;
    cg.close?.();
    return { edges, nodes };
  };
