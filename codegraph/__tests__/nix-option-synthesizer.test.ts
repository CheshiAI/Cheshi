...
describe('nix-option-path synthesizer', () => {
  ...
  async function synthEdges(d: string): Promise<any[]> {
    const cg = await CodeGraph.init(d);
    await cg.indexAll();
    const db = getCodeGraphState(cg).db.getDb();
    const rows = db
      .prepare(
        `SELECT s.name AS source, s.file_path AS sf, t.name AS target, t.file_path AS tf,
                json_extract(e.metadata,'$.optionPath') AS optionPath
