 ...
 *
 * The full-repo sweeps use `bun run kernel:parity` (shelf/bloc/flutter
 * with --max-deferral 0.3); this suite keeps the invariant alive in
 * `bun test`. Skips when no kernel binary is staged; CODEGRAPH_KERNEL_EXPECT=1
...
import type { ExtractionResult } from '../src';
