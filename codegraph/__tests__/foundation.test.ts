...
describe('CodeGraph Foundation', () => {
  ...
  describe('Recreate (#1067)', () => {
    ...
    it('throws a clear error when the project is not initialized', async () => {
      await expectRejection(CodeGraph.recreate(tempDir), (error) => {
        if (!(error instanceof Error)) throw new Error('Expected recreate to reject with an Error');
  ...
  describe('Close/Destroy', () => {
    ...
    it('close() keeps the project initialized while closing the database', () => {
      const cg = CodeGraph.initSync(tempDir);
  ...
  describe('Source extraction facade', () => {
    it('extractFromSource() exposes the extraction API without storing results', async () => {
      await loadGrammarsForLanguages(['typescript']);
      const cg = CodeGraph.initSync(tempDir);

      const result = cg.extractFromSource(
        'src/example.ts',
        'export function example() { return 1; }\n',
      );

      expect(result.nodes.some((node) => node.name === 'example')).toBe(true);
...
describe('Database Connection', () => {
  ...
  it('should preserve the original error when rollback is already complete', () => {
    const dbPath = path.join(tempDir, 'test.db');
    const db = DatabaseConnection.initialize(dbPath);
    ...
    })).toThrow('original transaction failure');
...
describe('central CodeGraph storage', () => {
  const savedDataRoot = process.env.CODEGRAPH_DATA_ROOT;
  let projectRoot: string;
  let dataRoot: string;

  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'codegraph-central-project-'));
    dataRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'codegraph-central-data-'));
    process.env.CODEGRAPH_DATA_ROOT = dataRoot;
  });

  afterEach(() => {
    if (savedDataRoot === undefined) delete process.env.CODEGRAPH_DATA_ROOT;
    else process.env.CODEGRAPH_DATA_ROOT = savedDataRoot;
  ...
  it('stores the database outside the source checkout without registering a desktop project', () => {
    ...
    try {
      ...
      expect(indexDirectory.startsWith(path.join(dataRoot, 'workspaces') + path.sep)).toBe(true);
      expect(path.basename(indexDirectory)).toBe('codegraph');
      expect(fs.existsSync(path.join(indexDirectory, 'codegraph.db'))).toBe(true);
  ...
  it('rejects a relative central data root instead of falling back into the project', () => {
    process.env.CODEGRAPH_DATA_ROOT = 'relative/codegraph-data';
    expect(() => getCodeGraphDir(projectRoot)).toThrow(/CODEGRAPH_DATA_ROOT must be an absolute path/);
    expect(fs.existsSync(path.join(projectRoot, '.codegraph'))).toBe(false);
  });
});
