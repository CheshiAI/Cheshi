 ...
 *
 * When a parent shim is killed, the indexer must not keep running orphaned and
 * pinning a CPU core. The `#850` liveness watchdog and `#277` ppid watchdog were
 * previously wired only into
...
const SUPERVISION = path.resolve(__dirname, '../src/bin/command-supervision.ts');
