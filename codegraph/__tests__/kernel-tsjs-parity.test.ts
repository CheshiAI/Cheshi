 ...
 *
 * The full-repo sweep uses `bun run kernel:parity` (excalidraw et al.,
 * run for the §5 gate); this suite keeps the invariant alive in `bun test`.
...
import type { ExtractionResult, Language } from '../src';
...
describe.skipIf(!kernelBuilt)('kernel TS/JS extraction parity', () => {
  ...
  it('preserves member receivers and named local closures', () => {
    assertParity('receivers.ts', `
export function createBrowser(options: any, contents: any) {
  const handle = (name: string) => options.ipc.handle(name);
  options.session.on('ready', () => handle('view'));
  contents.getURL();
  options.getContents().getURL();
}
class View { getURL() { return ''; } refresh() { return this.getURL(); } }
`, 'typescript');
  });
