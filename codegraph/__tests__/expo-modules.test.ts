...
import { expoModulesResolver } from '../src/resolution/frameworks';
...
`
    ...
    const callEdge = db
      .prepare(
        `SELECT t.name AS target, t.id AS target_id
         FROM edges AS e
         JOIN nodes AS s ON s.id = e.source
         JOIN nodes AS t ON t.id = e.target
         WHERE e.kind = 'calls'
...
`
    ...
    const pair = db.prepare(
      `SELECT count(*) AS c FROM edges AS e
