...
`
    ...
    const synth = db
      .prepare(
        `SELECT s.name AS source, t.name AS target, s.file_path AS sf, t.file_path AS tf
    ...
    const compToHook = db
      .prepare(
        `SELECT s.name AS source, t.name AS target FROM edges AS e
         JOIN nodes AS s ON s.id = e.source JOIN nodes AS t ON t.id = e.target
...
`
    ...
    const synth = db
      .prepare(`SELECT count(*) AS c FROM edges WHERE json_extract(metadata,'$.synthesizedBy') = 'rtk-query'`)
      .get();
    expect(synth.c).toBe(0);
    // The hand-written hook keeps its real body (not a sentinel binding).
    const sentinel = db
      .prepare(`SELECT count(*) AS c FROM nodes WHERE signature = '= RTK Query generated hook'`)
      .get();
