...
import CodeGraph, { DatabaseConnection, getCodeGraphDir } from '../src';
...
function writeRows(db: DatabaseConnection, rows: number): void {
  ...
  const insertStatement = ['INSERT INTO', 't', '(blob) VALUES (?)'].join(' ');
  const stmt = raw.prepare(insertStatement);
...
async function seedPendingRefs(cg: CodeGraph): Promise<void> {
  const raw = getCodeGraphState(cg).db.getDb();
...
describe('indexAll WAL deferral end-to-end', () => {
  ...
  it('produces the same graph with and without deferral, and restores the interval', async () => {
    ...
    const conn1 = getCodeGraphState(cg1).db;
...
describe('sync WAL deferral end-to-end (#1248)', () => {
  ...
  it('defers the autocheckpoint interval DURING sync and restores it after', async () => {
    ...
    await cg.indexAll();
    const conn = getCodeGraphState(cg).db;
    fs.writeFileSync(
  ...
  it('passes WAL backpressure through both resolution paths and stops the valve before maintenance', async () => {
    writeFixtureProject();
    const cg = CodeGraph.initSync(tmpDir);
    await cg.indexAll();
    const conn = getCodeGraphState(cg).db;
    const orchestrator = getCodeGraphState(cg).orchestrator;
    const originalResolve = cg.resolveReferencesBatched.bind(cg);
    const originalStop = WalCheckpointValve.prototype.stop;
    const originalMaintenance = conn.runMaintenance.bind(conn);
    const backpressureArgs: Array<(() => Promise<void> | null) | undefined> = [];
    const maintenanceSawStoppedValve: boolean[] = [];

    const backpressureSpy = spyOn(WalCheckpointValve.prototype, 'backpressure')
      .mockReturnValue(null);
    const stopSpy = spyOn(WalCheckpointValve.prototype, 'stop')
      .mockImplementation(function(this: WalCheckpointValve) {
        originalStop.call(this);
      });
    const resolutionSpy = spyOn(cg, 'resolveReferencesBatched')
      .mockImplementation((onProgress, onSynthesisProgress, backpressure) => {
        backpressureArgs.push(backpressure);
        return originalResolve(onProgress, onSynthesisProgress, backpressure);
      });
    const maintenanceSpy = spyOn(conn, 'runMaintenance').mockImplementation(async () => {
      maintenanceSawStoppedValve.push(stopSpy.mock.calls.length > 0);
      await originalMaintenance();
    });

    try {
      // Exercise the defensive unscoped branch directly. The current scanner
      // normally supplies changedFilePaths, but older/custom orchestrators may
      // report changed counts without that optional field.
      const unscopedSyncSpy = spyOn(orchestrator, 'sync').mockResolvedValue({
        filesChecked: 1,
        filesAdded: 0,
        filesModified: 1,
        filesRemoved: 0,
      ...
      try {
        ...
        unscopedSyncSpy.mockRestore();
      }
      expect(backpressureArgs.length).toBeGreaterThan(0);
      expect(backpressureArgs.every((callback) => typeof callback === 'function')).toBe(true);

      // With no file changes, seeded pending references can only be consumed by
      // the orphan sweep. It must receive the same valve callback.
      const callsBeforeOrphanSweep = backpressureArgs.length;
      await seedPendingRefs(cg);
      await cg.sync();
      const orphanSweepArgs = backpressureArgs.slice(callsBeforeOrphanSweep);
      expect(orphanSweepArgs.length).toBeGreaterThan(0);
      expect(orphanSweepArgs.every((callback) => typeof callback === 'function')).toBe(true);

      expect(backpressureSpy.mock.calls.length).toBeGreaterThan(0);
      expect(maintenanceSawStoppedValve.length).toBeGreaterThanOrEqual(2);
      expect(maintenanceSawStoppedValve.every(Boolean)).toBe(true);
    } finally {
...
describe('resolution-phase WAL backpressure plumbing (§7a.1)', () => {
  ...
  it('parks Git-scoped resolution while retaining the FileLock until cleanup', async () => {
    writeFixtureProject();
    const cg = CodeGraph.initSync(tmpDir);
    await cg.indexAll();
    const conn = getCodeGraphState(cg).db;
    const raw = conn.getDb();
    const node = raw.prepare(
      "SELECT id FROM nodes WHERE file_path = 'src/mod0.ts' AND kind = 'function' LIMIT 1"
    ).get() as { id: string } | undefined;
    expect(node).toBeDefined();
    raw.prepare(
      "INSERT INTO unresolved_refs (from_node_id, reference_name, reference_kind, line, col, file_path, language, status) VALUES (?, 'missing0', 'calls', 1, 0, 'src/mod0.ts', 'typescript', 'pending')"
    ).run(node!.id);

    // The scanner normally supplies changedFilePaths for the Git fast path.
    // Pin that branch directly so the test is deterministic and independent of
    // filesystem timestamp resolution or the host Git installation.
    const orchestrator = getCodeGraphState(cg).orchestrator;
    const syncSpy = spyOn(orchestrator, 'sync').mockResolvedValue({
      filesChecked: 1,
      filesAdded: 0,
      filesModified: 1,
      filesRemoved: 0,
      nodesUpdated: 0,
      durationMs: 1,
      changedFilePaths: ['src/mod0.ts'],
    });
    const lockPath = path.join(getCodeGraphDir(tmpDir), 'codegraph.lock');

    let releaseGate!: () => void;
    const gate = new Promise<void>((resolve) => { releaseGate = resolve; });
    let backpressureHit = false;
    let signalBackpressureHit!: () => void;
    const backpressureHitPromise = new Promise<void>((resolve) => {
      signalBackpressureHit = resolve;
    });
    let settled = false;
    let done: ReturnType<CodeGraph['sync']> | undefined;
    const backpressureSpy = spyOn(WalCheckpointValve.prototype, 'backpressure')
      .mockImplementation(() => {
        if (backpressureHit) return null;
        backpressureHit = true;
        signalBackpressureHit();
        return gate;
      });

    try {
      done = cg.sync().finally(() => { settled = true; });
      await new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error('timed out waiting for Git-scoped WAL backpressure'));
        }, 2000);
        void backpressureHitPromise.then(() => {
      ...
      expect(result.filesModified).toBe(1);
      expect(result.changedFilePaths).toContain('src/mod0.ts');
      expect(conn.getWalAutocheckpoint()).toBe(1000);
      expect(fs.existsSync(lockPath)).toBe(false);
    } finally {
      releaseGate();
      if (done && !settled) await done.catch(() => undefined);
      syncSpy.mockRestore();
      backpressureSpy.mockRestore();
      cg.close();
    }
