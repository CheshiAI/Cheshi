 ...
 * fresh `init`.
 *
 * Exercised end-to-end against the Bun source CLI so the wiring (not just the
 * library) is covered.
 */
...
import { CodeGraph, DatabaseConnection, FileLock, getCodeGraphDir } from '../src';
...
describe('codegraph index — full re-index keeps the graph populated (#874)', () => {
  ...
  it('does not delete the existing database when the project write lock is unavailable', async () => {
    runCodegraph(['init'], tempDir);
  ...
  it('retains the recreation lock until indexAll finishes', async () => {
    runCodegraph(['init'], tempDir);
    const rebuilt = await CodeGraph.recreate(tempDir);
    const contender = new FileLock(path.join(getCodeGraphDir(tempDir), 'codegraph.lock'));
    try {
      expect(() => contender.acquire()).toThrow(/locked by another process/);
      const result = await rebuilt.indexAll();
