...
import { DatabaseConnection, QueryBuilder, type Edge, type Node } from '../src';
import type { SqliteDatabase } from '../src/db';
import { runMigrations, getCurrentVersion } from '../src/db/migrations';

const EDGE_IDENTITY_INDEX_NAME = 'idx_edges_identity';

interface SqliteIndexListRow {
  name: string;
}

function hasEdgeIdentityIndex(db: SqliteDatabase): boolean {
  const indexes = db
    .prepare('PRAGMA index_list(edges)')
    .all() as SqliteIndexListRow[];
  return indexes.some(({ name }) => name === EDGE_IDENTITY_INDEX_NAME);
}
...
describe('deleteResolvedReferences (chunking)', () => {
  ...
  it('deletes unresolved refs for more ids than the SQLite parameter limit (#1001)', () => {
    ...
    // IN (...), so passing more ids than SQLITE_MAX_VARIABLE_NUMBER (32766 in
    // the tested bun:sqlite build) threw "too many SQL variables". Use 33000 to
...
describe('runMaintenance', () => {
  ...
  it('runs without throwing on a fresh database', async () => {
    expect(await db.runMaintenance()).toBeUndefined();
  });
...
describe('edge identity uniqueness (#1034)', () => {
  ...
  it('a fresh database has the identity index', () => {
    expect(hasEdgeIdentityIndex(db.getDb())).toBe(true);
  });
...
describe('migration v6: dedup edges + add identity index on upgrade (#1034)', () => {
  it('collapses pre-existing duplicate rows, keeps distinct ones, and restores the constraint', () => {
    ...
    expect(hasEdgeIdentityIndex(raw)).toBe(true);
