...
`);
    ...
    const edges = db
      .prepare(
        `SELECT s.name AS source, t.name AS target, json_extract(e.metadata,'$.via') AS via
...
`);
    ...
    const db = getCodeGraphState(cg).db.getDb();
    const count = db
      .prepare(`SELECT count(*) AS c FROM edges WHERE json_extract(metadata,'$.synthesizedBy') = 'mediatr-dispatch'`)
      .get();
