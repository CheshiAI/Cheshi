import { getToolHandlerState } from './helpers/internal-state';
/**
 * Issue #238 — "database is locked" on concurrent MCP tool calls.
 *
 * With bun:sqlite (real WAL) as the backend, the fixes that remain relevant:
...
describe('issue #238 — ToolHandler reuses the default instance (#2)', () => {
  ...
  it('getCodeGraph(defaultRoot) returns the default instance, not a new connection', () => {
    const openSpy = spyOn(CodeGraph, 'openSync');
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const resolved = getToolHandlerState(handler).getCodeGraph(root);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const nested = getToolHandlerState(handler).getCodeGraph(path.join(root, 'does', 'not', 'exist'));
