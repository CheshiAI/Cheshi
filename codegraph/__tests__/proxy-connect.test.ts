...
import { CodeGraphPackageVersion } from '../src/mcp';
...
describe('connectWithHello — socket is never left without an error listener (#974)', () => {
  it.if(process.platform !== 'win32')('returns a socket that has an error listener and never throws on error', async () => {
  ...
  it.if(process.platform !== 'win32')('still reports version-mismatch (and that path does not throw)', async () => {
    const { sockPath } = await fakeDaemon('0.0.0-not-our-version');
    const result = await connectWithHello(sockPath);
    expect(result).toBe('version-mismatch');
  });

  it.if(process.platform !== 'win32')('returns null when no daemon is listening', async () => {
