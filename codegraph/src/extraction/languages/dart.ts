...
function dartLibraryUri(node: SyntaxNode, source: string): string | undefined {
  const configurableUri = node.namedChildren.find((c: SyntaxNode) => c.type === 'configurable_uri');
  ...
  return stringLiteral ? getNodeText(stringLiteral, source).replace(/['"]/g, '') : undefined;
}
...
export const dartExtractor: LanguageExtractor = {
  ...
  extractImport: (node, source) => {
    ...
    if (libraryImport) {
      ...
      if (importSpec) {
        moduleName = dartLibraryUri(importSpec, source) ?? '';
      }
    }
    ...
    if (!moduleName) {
      ...
      if (libraryExport) {
        moduleName = dartLibraryUri(libraryExport, source) ?? '';
      }
    }
