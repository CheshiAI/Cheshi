...
import { embeddedBlockRanges, embeddedContentStartLine } from './embedded-blocks';
...
export class SvelteExtractor {
  ...
  private extractScriptBlocks(): Array<{
    ...
    while ((match = scriptRegex.exec(this.source)) !== null) {
      ...
      const contentStartLine = embeddedContentStartLine(this.source, match.index, match[0]);
  ...
  private extractTemplateCalls(
    ...
    const coveredRanges = embeddedBlockRanges(this.source, /<(script|style)(\s[^>]*)?>[\s\S]*?<\/\1>/g);
