 ...
 *   2. <up3>/kernel/codegraph-kernel.node — a compatible staged layout
 ...
 *
 * "up3" = three directories above this source file, which is the package root.
 */
...
export interface KernelModule {
  ...
  /** Explicit runtime configuration supported by current kernels. Older
   *  prebuilds expose only extractFile and are handled by the TS fallback. */
  extractFileConfigured?(
    filePath: string,
    content: string,
    language: string,
    valueRefsEnabled: boolean,
  ): KernelBuffers;
