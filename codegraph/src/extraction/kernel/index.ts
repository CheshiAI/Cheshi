...
import { getKernel, kernelSupports, type KernelBuffers, type KernelModule } from './loader';
...
/**
 * Languages routed to the kernel by default after passing parity gates.
...
const DEFAULT_ROUTED: ReadonlySet<Language> = new Set<Language>([
  ...
  // `bun run kernel:parity -- --max-deferral ...`).
  'c',
  'cpp',
...
function assertKernelAbi(meta: Buffer): void {
  const version = meta.readUInt8(LAYOUT_META.version);
  if (version !== LAYOUT_ABI) {
...
/**
 * Extract with the runtime feature configuration passed explicitly across the
 * N-API boundary. Bun's process.env mutations are visible to this TS process,
 * but a Rust std::env read in an already-loaded addon may still see the launch
 * environment. Current kernels therefore receive the value directly.
 *
 * An older compatible prebuild has no configured entry point. When value refs
 * are disabled, fall back to WASM rather than silently emitting the feature;
 * with the default enabled configuration the legacy fast path remains valid.
 */
function extractKernelFile(
  kernel: KernelModule,
  filePath: string,
  source: string,
  language: Language,
): KernelBuffers | null {
  const valueRefsEnabled = process.env.CODEGRAPH_VALUE_REFS !== '0';
  if (kernel.extractFileConfigured) {
    return kernel.extractFileConfigured(filePath, source, language, valueRefsEnabled);
  }
  if (!valueRefsEnabled) return null;
  return kernel.extractFile(filePath, source, language);
}
...
export function tryKernelExtractRaw(
  ...
  try {
    const buffers = extractKernelFile(kernel, filePath, pre, language);
    if (!buffers) return null;
    const meta = buffers.meta;
    assertKernelAbi(meta);
    const counts = {
...
export function tryKernelExtract(
  ...
  try {
    ...
    // `defer:` is the kernel's expected-routing signal; the fallback keeps
    // wasm's canonical error-recovery behavior.
    return handleKernelFailure(err, filePath, source, language, pre);
  }
}
