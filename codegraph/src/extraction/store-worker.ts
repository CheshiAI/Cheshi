...
import { decodeExtractBuffers } from './kernel';
...
function requireStoreQueries(value: QueryBuilder | null): QueryBuilder {
  if (!value) throw new Error('store-worker: bundle before open');
  return value;
}
  ...
  try {
    switch (msg.type) {
      case 'open': {
        ...
        if (msg.fastInit) {
          // The main connection keeps WAL enabled during fast init. MEMORY
          // journal mode is connection-local and can corrupt the hand-off to
          // the resolver when this worker closes its fresh-build connection.
      ...
      case 'bundle': {
        const activeQueries = requireStoreQueries(queries);
        if (STORE_TIMINGS) {
          bundleCount++;
          ...
          activeQueries.storeFileBundle(bundle);
          decodeNs += t1 - t0;
