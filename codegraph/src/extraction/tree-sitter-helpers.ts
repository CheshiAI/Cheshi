...
export function resolveWrappedFunctionBody(node: SyntaxNode, bodyField: string): SyntaxNode | null {
...
function cleanCommentMarkers(comment: string): string {
  ...
  else if (c.startsWith('--[')) c = c.replace(/^--\[=*\[/, '').replace(/\x5D=*\x5D$/, '');
  else if (c.startsWith('(*')) c = c.replace(/^\(\*/, '').replace(/\*\)$/, '');
  else if (c.startsWith('{')) c = c.replace(/^\{/, '').replace(/\x7D$/, '');
  return c
