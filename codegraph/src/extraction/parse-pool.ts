 ...
 * queue / growth / recycle / crash-recovery logic without spawning threads.
 */
 ...
 * issue #1231) the runtime services an expired timer before the queued worker
...
export interface ParseWorkerPoolOptions {
  ...
  size: number;
  /** Bun-executable `parse-worker.ts` path. Required unless `createWorker` is given. */
...
export class ParseWorkerPool {
   ...
   * of the parse phase (each worker boot is a fresh Bun worker + grammar
   ...
   * synchronous main-thread stretch an expired timer can run before an
   * already-delivered `parse-result`, so the result is still queued behind
