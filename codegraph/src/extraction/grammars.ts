 ...
 * are compiled, keeping WASM memory pressure low on large codebases.
 */
...
export async function initGrammars(): Promise<void> {
  ...
  const runtimeWasmPath = process.env.CODEGRAPH_TREE_SITTER_WASM?.trim();
  await Parser.init(runtimeWasmPath
...
/**
 * Grammars that ship their own vendored WASMs under `src/extraction/wasm/`
...
function resolveWasmPath(lang: GrammarLanguage): string {
  ...
  const runtimePath = codeGraphRuntimePath('grammars', wasmFile);
...
export async function loadGrammarsForLanguages(languages: Language[], wasmBytes?: Record<string, Uint8Array>): Promise<void> {
  ...
  // Load grammars sequentially to avoid web-tree-sitter WASM initialization races.
