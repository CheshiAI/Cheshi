 ...
 * tests (the reason the previous chokidar mock existed). Only populated under
...
export interface WatchOptions {
   ...
   * FSEvents/inotify delivery races under parallel tests). Production never
   * sets it.
   */
...
export class FileWatcher {
  ...
  private pendingFiles = new Map<
    string,
    { firstSeenMs: number; lastSeenMs: number; eventSequence: number }
  >();
  /**
   * Monotonic event generations avoid using millisecond timestamps as a
   * correctness boundary: an event can arrive mid-sync in the exact same
   * millisecond the sync started. Timestamp comparisons would then erase that
   * newer event as though the in-flight sync had absorbed it.
   */
  private eventSequence = 0;
  private syncStartedSequence = 0;
   ...
   * only after sync commits successfully, and only for entries whose event
   * generation is at or before the sync's start generation. That way, a query
   * that arrives mid-sync
  ...
  private async flush(): Promise<void> {
    ...
    this.syncStartedSequence = this.eventSequence;
    this.syncing = true;
    ...
    try {
      ...
      // edits are now in the DB. Entries with a generation newer than the
      // start boundary arrived mid-sync; whether the in-flight sync captured
      // them depends
      ...
      for (const [filePath, info] of this.pendingFiles) {
        if (info.eventSequence <= this.syncStartedSequence) {
   ...
   *
   * `indexing` is true when a sync is currently in flight whose start generation
   * is at or after this file's most recent event — i.e. that sync will absorb the
  ...
  getPendingFiles(): PendingFile[] {
    ...
    for (const [filePath, info] of this.pendingFiles) {
      result.push({
        ...
        indexing: this.syncing && this.syncStartedSequence >= info.eventSequence,
      });
    }
    return result;
  }
 ...
 * parallel tests). `relPath` is project-relative POSIX (e.g. "src/foo.ts").
