...
function pickNativeNode<T extends { filePath: string }>(candidates: T[], filePath: string): T | undefined {
  return candidates.find((candidate) => candidate.filePath === filePath) ?? candidates[0];
}
  ...
  for (const file of allFiles) {
    ...
    if (file.endsWith('.m') || file.endsWith('.mm')) {
      ...
      for (const exp of exports) {
        ...
        const node = pickNativeNode(candidates, file);
        if (!node) continue;
