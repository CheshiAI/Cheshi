...
import { resolveByNameAndKind } from './symbol-resolver';
...
export const aspnetResolver: FrameworkResolver = {
  ...
  extract(filePath, content) {
    ...
    let classPrefix = '';
    const cls = /\[Route\s*\(\s*"([^"]+)"[^)]*\)\x5D\s*(?:\[[^\x5D]*\x5D\s*)*(?:public\s+|sealed\s+|abstract\s+|partial\s+)*class\b/.exec(safe);
    ...
    const attrRegex = /\[(HttpGet|HttpPost|HttpPut|HttpPatch|HttpDelete)(?:\s*\(\s*"([^"]+)"[^)]*\))?\s*\x5D/g;
