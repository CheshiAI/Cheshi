...
import { isPascalCaseName, resolveComponentByName } from './component-resolver';
...
export const svelteResolver: FrameworkResolver = {
  ...
  resolve(ref: UnresolvedRef, context: ResolutionContext): ResolvedRef | null {
    ...
    if (isPascalCaseName(ref.referenceName) && ref.referenceKind === 'calls') {
...
function isRuneReference(name: string): boolean {
  ...
  return name === '$state' || name === '$derived' || name === '$effect';
}
...
function resolveComponent(
  ...
  return resolveComponentByName(name, fromFile, context);
}
...
function filePathToSvelteKitRoute(filePath: string): string | null {
  ...
  let route = '/' + dirPath
    .replace(/\[\.\.\.([^\x5D]+)\x5D/g, '*$1')  // [...rest] -> *rest
    .replace(/\[{2}([^\x5D]+)\x5D{2}/g, ':$1?') // [[optional]] -> :optional?
    .replace(/\[([^\x5D]+)\x5D/g, ':$1');        // [param] -> :param
