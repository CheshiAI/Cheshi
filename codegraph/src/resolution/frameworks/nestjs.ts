...
function readArgs(s: string, openIndex: number): { args: string; end: number } | null {
  ...
  const close = matchingClose(s, openIndex);
  return close < 0 ? null : { args: s.slice(openIndex + 1, close), end: close + 1 };
}

function skipWhitespace(s: string, index: number): number {
  while (index < s.length && /\s/.test(s[index]!)) index++;
  return index;
}

function skipDecorators(safe: string, start: number): number | null {
  let i = start;
  const decoName = /@[\w.]+/y;
  for (;;) {
    i = skipWhitespace(safe, i);
    if (safe[i] !== '@') return i;
    decoName.lastIndex = i;
    if (!decoName.exec(safe)) return i;
    i = skipWhitespace(safe, decoName.lastIndex);
    if (safe[i] !== '(') continue;
...
function methodNameAfter(safe: string, start: number): string | null {
  const decoratorEnd = skipDecorators(safe, start);
  if (decoratorEnd === null) return null;
  let i = decoratorEnd;
...
function scanStringAware(
  s: string,
  start: number,
  visit: (index: number, character: string) => boolean,
): number {
...
function matchingClose(s: string, open: number): number {
  ...
  let depth = 0;
  return scanStringAware(s, open, (_index, character) => {
...
function findNextObjectStart(s: string, start: number): number {
  return scanStringAware(s, start, (_index, character) => character === '{');
}
...
function splitTopLevelObjects(s: string): string[] {
  ...
  while (cursor < s.length) {
    const open = findNextObjectStart(s, cursor);
    if (open < 0) break;
