...
export class ResolverPool {
  ...
  /**
   * Create a pool when the Bun-executable source worker exists, the kill switch is
  ...
  static tryCreate(dbPath: string, projectRoot: string): ResolverPool | null {
    ...
    const workerScript = codeGraphRuntimePath('workers', 'resolver-worker.js')
      ?? path.join(__dirname, 'resolver-worker.ts');
