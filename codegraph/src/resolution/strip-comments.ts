...
function skipQuoted(
  ...
  let i = start + 1;
  while (i < src.length) {
    if (escapes && src[i] === '\\' && i + 1 < src.length) {
      i += 2;
      continue;
    }
...
interface SlashCommentOptions {
  allowSingleQuoteStrings?: boolean;
  hashComments?: boolean;
  rawQuotes?: string;
  newlineQuotes?: string;
}

function stripSlashComments(src: string, options: SlashCommentOptions): string {
  const out = src.split('');
  let i = 0;
  while (i < src.length) {
    const c = src[i]!;
    ...
    if (options.hashComments && c === '#') {
      const start = i;
      while (i < src.length && src[i] !== '\n') i++;
      blankRange(out, start, i, src);
      continue;
    }
    const isQuote = c === '"' || c === '`' || (options.allowSingleQuoteStrings && c === "'");
    if (isQuote) {
      i = skipQuoted(
        src,
        i,
        c,
        options.newlineQuotes?.includes(c) ?? false,
        !options.rawQuotes?.includes(c),
      );
      continue;
    }
    i++;
