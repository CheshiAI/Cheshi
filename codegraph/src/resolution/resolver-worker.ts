...
function requireResolver(value: ReferenceResolver | null, reason: string): ReferenceResolver {
  if (!value) throw new Error(`resolver-worker: ${reason} before open`);
  return value;
}

function requireResolverSession(
  value: QueryBuilder | null,
  pathValue: string | null,
): { queries: QueryBuilder; dbPath: string } {
  if (!value || !pathValue) throw new Error('resolver-worker: recycle before open');
  return { queries: value, dbPath: pathValue };
}

function requireSynthPass(name: string): (typeof SYNTH_PASSES)[number] {
  const pass = SYNTH_PASSES.find((candidate) => candidate.name === name);
  if (!pass) throw new Error(`resolver-worker: unknown synth pass '${name}'`);
  return pass;
}
  ...
  try {
    switch (msg.type) {
      ...
      case 'recycle': {
        ...
        const session = requireResolverSession(queries, dbPath);
        try {
          db?.close();
        } catch { /* already closed */ }
        const reopened = createDatabase(session.dbPath, { readOnly: true });
        db = reopened.db;
        db.pragma('busy_timeout = 5000');
        db.pragma('cache_size = -32000');
        session.queries.rebind(db);
        port.postMessage({ type: 'recycled', id: msg.id });
        break;
      }
      case 'resolve': {
        const activeResolver = requireResolver(resolver, 'resolve');
        const tRes = Date.now();
        const out = activeResolver.resolveListForAdmission(msg.refs);
      ...
      case 'synth': {
        ...
        const q = requireResolverSession(queries, dbPath).queries;
        const r = requireResolver(resolver, 'synth');
        const pass = requireSynthPass(msg.pass);
        void (async () => {
          const t0 = Date.now();
          try {
