...
import { createDatabase, type SqliteBackend, type SqliteDatabase } from './sqlite-adapter';
...
function resolveSchemaPath(): string {
  return codeGraphRuntimePath('schema.sql') ?? path.join(__dirname, 'schema.sql');
}
 ...
 */
function configureConnection(db: SqliteDatabase, readOnly: boolean): void {
  db.pragma('busy_timeout = 5000');      // MUST be first — see above
  db.pragma('foreign_keys = ON');
  if (readOnly) {
    // Defense in depth over the adapter's SQLITE_OPEN_READONLY flag. Do not run
    // journal-mode, migration, recovery, checkpoint, or maintenance writes on
    // this path: OpenOptions.readOnly promises an observational connection.
...
function assertMigrationAllowed(currentVersion: number, readOnly: boolean): void {
  if (!readOnly) return;
  throw new Error(
    `Database schema ${currentVersion} requires migration to ${CURRENT_SCHEMA_VERSION}; ` +
      'open it without readOnly once before using a read-only connection.'
  );
}
...
export interface DatabaseOpenOptions {
  /** Open with SQLite's read-only flag and skip all write-side recovery work. */
  readOnly?: boolean;
}

const FTS_TABLE = 'nodes_fts';
const FTS_CONTENT_TABLE = 'nodes';
const FTS_COLUMNS = ['rowid', 'id', 'name', 'qualified_name', 'docstring', 'signature'] as const;

const FTS_TRIGGER_SPECS = [
  { name: 'nodes_ai', event: 'INSERT', operations: ['insert'] },
  { name: 'nodes_ad', event: 'DELETE', operations: ['delete'] },
  { name: 'nodes_au', event: 'UPDATE', operations: ['delete', 'insert'] },
] as const;

const FTS_TRIGGER_NAMES = FTS_TRIGGER_SPECS.map(({ name }) => name);

type FtsRowAlias = 'NEW' | 'OLD';

function buildFtsRowValues(alias: FtsRowAlias): string[] {
  return FTS_COLUMNS.map((column) => `${alias}.${column}`);
}

function buildFtsInsert(alias: FtsRowAlias): string {
  return [
    'INSERT INTO',
    FTS_TABLE,
    `(${FTS_COLUMNS.join(', ')})`,
    'VALUES',
    `(${buildFtsRowValues(alias).join(', ')})`,
  ].join(' ');
}

function buildFtsDelete(): string {
  return [
    'INSERT INTO',
    FTS_TABLE,
    `(${[FTS_TABLE, ...FTS_COLUMNS].join(', ')})`,
    'VALUES',
    `(${["'delete'", ...buildFtsRowValues('OLD')].join(', ')})`,
  ].join(' ');
}

function buildFtsTriggerStatements(): string[] {
  const statements = {
    insert: buildFtsInsert('NEW'),
    delete: buildFtsDelete(),
  } as const;

  return FTS_TRIGGER_SPECS.map(({ name, event, operations }) =>
    [
      `CREATE TRIGGER IF NOT EXISTS ${name} AFTER ${event} ON ${FTS_CONTENT_TABLE} BEGIN`,
      ...operations.map((operation) => `${statements[operation]};`),
      'END;',
    ].join('\n')
  );
}

function installFtsTriggers(db: SqliteDatabase): void {
  for (const statement of buildFtsTriggerStatements()) {
    db.exec(statement);
  }
}

function buildFtsControlStatement(): string {
  return ['INSERT INTO', FTS_TABLE, `(${FTS_TABLE})`, 'VALUES (?)'].join(' ');
}

function buildFtsTriggerCountStatement(): string {
...
export class DatabaseConnection {
  ...
  static open(dbPath: string, options: DatabaseOpenOptions = {}): DatabaseConnection {
    ...
    // A non-empty WAL contains committed state newer than the main DB and must
    // be replayed by a normal read-only connection. With no WAL frames, open an
    // immutable snapshot so SQLite never needs to create or modify sidecars —
    // this is what makes readOnly usable on a read-only filesystem.
    let hasWalFrames = false;
    if (readOnly) {
      try { hasWalFrames = fs.statSync(`${dbPath}-wal`).size > 0; } catch { /* no WAL */ }
    }
    const { db, backend } = createDatabase(dbPath, {
      readOnly,
      immutable: readOnly && !hasWalFrames,
    });

    try {
      configureConnection(db, readOnly);

      // Check and run migrations if needed. A read-only open must never upgrade
      // schema state behind the caller's back; fail with an actionable error.
      const conn = new DatabaseConnection(db, dbPath, backend, readOnly);
      const currentVersion = getCurrentVersion(db);

      if (currentVersion < CURRENT_SCHEMA_VERSION) {
        assertMigrationAllowed(currentVersion, readOnly);
      ...
      try { db.close(); } catch { /* preserve the open/migration error */ }
      throw error;
    }
  }
  ...
  beginBulkNodeLoad(): void {
    for (const t of FTS_TRIGGER_NAMES) {
   ...
   * idempotent synchronization triggers.
   */
  endBulkNodeLoad(): void {
    this.db.prepare(buildFtsControlStatement()).run('rebuild');
  ...
  private healBulkNodeLoad(): void {
    const row = this.db
      .prepare(buildFtsTriggerCountStatement())
      .get(...FTS_TRIGGER_NAMES) as { c: number } | undefined;
    if ((row?.c ?? 0) >= FTS_TRIGGER_NAMES.length) return;
    this.endBulkNodeLoad();
  }

  /** Recreate the generated FTS synchronization triggers. */
  private recreateFtsTriggers(): void {
    installFtsTriggers(this.db);
  }
  ...
  /** Whether this connection was opened with SQLite's read-only flag. */
  isReadOnly(): boolean {
    ...
    try {
      ...
      const workerSource = `
        ...
        try {
          ...
          const db = new Database(workerData.dbPath, { strict: true });
   ...
   */
  optimize(): void {
    if (this.readOnly) throw new Error('Cannot optimize a read-only database connection');
    this.db.exec('VACUUM');
