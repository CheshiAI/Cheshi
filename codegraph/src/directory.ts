...
/**
 * Get the CodeGraph data directory for a project.
 *
 * Cheshi sets `CODEGRAPH_DATA_ROOT` to its user-data directory. In that mode,
 * indexes live outside source checkouts at
 * `<data-root>/workspaces/<workspace-id>/codegraph`. Without the environment
 * variable, the standalone engine keeps its original per-project behavior.
 */
export function getCodeGraphDir(projectRoot: string): string {
  const dataRoot = resolveCodeGraphDataRoot();
  if (dataRoot) return codeGraphStorageDirectory(dataRoot, projectRoot);
  return path.join(projectRoot, codeGraphDirName());
}

function usesCentralStorage(): boolean {
  return resolveCodeGraphDataRoot() !== null;
}
...
export function createDirectory(projectRoot: string): void {
  ...
  if (!dataRoot) {
    // Project-local indexes need a .gitignore. Central Cheshi data lives
    // outside the checkout and therefore must not contain repository policy.
...
/**
 * Check if the .codegraph directory has valid structure. Read-only validation
 * skips the best-effort `.gitignore` repair so opening cannot mutate media.
 */
export function validateDirectory(
  ...
  if (options.readOnly !== true && !usesCentralStorage()) {
