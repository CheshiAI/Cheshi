 ...
 * @param [options.allowSymlinkEscape=false] - Follow in-root symlinks out of the root
...
export class FileLock {
  ...
  /**
   * A freshly-created lock can be observed between O_EXCL creation and the PID
   * write. Fail closed during this grace period instead of deleting a live
   * writer's temporarily empty/malformed lock. Valid live-PID locks never
   * expire by age; long-running indexes routinely exceed this interval.
   */
  private static readonly MALFORMED_LOCK_GRACE_MS = 2 * 60 * 1000;
  ...
  acquire(): void {
    ...
    if (fs.existsSync(this.lockPath)) {
      ...
      try {
        ...
        const parsedPid = /^\d+$/.test(content) ? Number(content) : NaN;
        ...
        } else if (pid === null && lockAge < FileLock.MALFORMED_LOCK_GRACE_MS) {
          lockError = new Error(
            'CodeGraph database lock is being created or is malformed. ' +
            `If this persists, run 'codegraph unlock' or delete ${this.lockPath}`
          );
        } else {
          // A valid dead PID is stale immediately. A malformed lock is removed
          // only after the creation grace period above.
          fs.unlinkSync(this.lockPath);
        }
      } catch (error) {
        // The lock may disappear between exists/read/stat when its owner exits.
        // Any other inspection failure is ambiguous, so fail closed instead of
        // deleting a lock we cannot prove stale.
        if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
          const reason = error instanceof Error ? error.message : String(error);
          lockError = new Error(`Could not inspect CodeGraph database lock: ${reason}`);
        }
      }
  ...
  /** True only while this FileLock instance still owns the on-disk lock. */
  isHeld(): boolean {
    if (!this.held) return false;
    try {
      if (fs.readFileSync(this.lockPath, 'utf-8').trim() === String(process.pid)) {
        return true;
      }
    } catch {
      // Removed/replaced lock: fall through and clear our local ownership bit.
    }
    this.held = false;
    return false;
  }
...
export class MemoryMonitor {
  ...
  private readonly onThresholdExceeded?: (usage: number) => void;
