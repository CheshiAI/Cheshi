...
export async function runInstallerWithOptions(opts: RunInstallerOptions): Promise<void> {
  ...
  clack.intro(`CodeGraph v${CodeGraphPackageVersion}`);
  ...
  // they're committing to before we write any configuration. Detection
  ...
  // Building the per-project graph is the user's explicit init command
  ...
  const initCommand = codeGraphCliCommand(['init']);
  clack.note(
    location === 'local'
      ? `${initCommand}        # build this project’s graph (one time; auto-syncs after)`
      : `cd <your-project>\n${initCommand}        # build a project’s graph (one time; auto-syncs after)`,
...
export async function runUninstaller(opts: RunUninstallerOptions): Promise<void> {
  ...
  clack.intro(`CodeGraph v${CodeGraphPackageVersion} — uninstall`);
 ...
 * (commit / pull / checkout) instead of a manual sync command.
 *
...
export async function offerWatchFallback(
  ...
  const syncCommand = codeGraphCliCommand(['sync']);
  ...
  if (!isGitRepo(projectPath)) {
    clack.log.info(`Run \`${syncCommand}\` after changing files to refresh the index.`);
    return;
  }
  ...
  if (opts.yes) {
    ...
    const sel = await clack.select({
      ...
      options: [
        ...
        { value: 'manual' as const, label: `I'll run \`${syncCommand}\` myself`, hint: 'fully manual' },
      ],
      initialValue: 'hook' as const,
    });
    if (clack.isCancel(sel)) {
      clack.log.info(`Skipped — run \`${syncCommand}\` after changes to refresh the index.`);
      return;
    }
    choice = sel;
  }
  ...
  if (result.installed.length > 0) {
    ...
    clack.log.info(`Run \`${syncCommand}\` anytime to refresh immediately.`);
  } else {
    clack.log.warn(
      `Could not install git hooks${result.skipped ? ` (${result.skipped})` : ''}. ` +
      `Run \`${syncCommand}\` after changes instead.`,
    );
  }
}
