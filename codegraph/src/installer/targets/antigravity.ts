 ...
 */
function resolveCodegraphCommand(command: string): string {
...
function buildAntigravityEntry(): { command: string; args: string[] } {
  ...
  return {
    command: resolveCodegraphCommand(invocation.command),
...
function writeMcpEntry(): WriteResult['files'][number] {
  ...
  return { path: file, action: writeMcpServerEntry(file, buildAntigravityEntry()) };
}
