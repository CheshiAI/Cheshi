...
// Import the concrete ESM entry so Bun's single-file compiler follows and
// embeds jsonc-parser's relative implementation modules. Importing the package
// root selects its UMD entry during `bun build --compile`, which leaves runtime
// `./impl/*` requires unresolved inside the compiled cheshi-cli executable.
import { parse as parseJsonc, modify, applyEdits } from 'jsonc-parser/lib/esm/main.js';
import {
  AgentTarget,
  DetectionResult,
  InstallOptions,
