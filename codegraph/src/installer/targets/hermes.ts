...
function childStartPattern(child: string): RegExp {
  return new RegExp(`^\\x20{2}${escapeRegExp(child)}:\\s*(?:#.*)?$`);
}

function findChildStart(lines: string[], parent: LineRange, child: string): number {
  const startPattern = childStartPattern(child);
  for (let i = parent.start + 1; i < parent.end; i++) {
    if (startPattern.test(lines[i] ?? '')) return i;
  }
  return -1;
}
...
function childRange(lines: string[], parent: LineRange, child: string): LineRange | null {
  const start = findChildStart(lines, parent, child);
  if (start === -1) return null;
...
function listChildBlock(
  ...
  for (let i = start + 1; i < parent.end; i++) {
    ...
    if (indent === 2 && /^\x20{2}- /.test(line)) continue;
    end = i;
    break;
  }
  while (end > start + 1 && (lines[end - 1] ?? '').trim() === '') {
...
function renderCodeGraphMcpChild(): string[] {
  ...
  return [
    ...
    '    args:',
    ...invocation.args.map((argument) => `      - ${argument}`),
    '    timeout: 120',
    '    connect_timeout: 60',
    '    enabled: true',
  ];
