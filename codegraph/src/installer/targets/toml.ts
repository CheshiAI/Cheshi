 ...
 * supported: string, string[], and string maps. Maps are emitted as inline
 * tables so a single owned MCP block remains atomic during install/uninstall.
 */
export type TomlTableValue = string | string[] | Record<string, string>;

export function serializeTomlTableBody(values: Record<string, TomlTableValue>): string {
  const lines: string[] = [];
  for (const [key, value] of Object.entries(values)) {
    if (typeof value === 'string') {
      ...
      if (!entries.every(([, entryValue]) => typeof entryValue === 'string')) {
        throw new Error(`Unsupported TOML map value type for key "${key}"`);
      }
      const fields = entries
        .map(([entryKey, entryValue]) => `${quoteKey(entryKey)} = ${quoteString(entryValue)}`)
        .join(', ');
...
function quoteKey(key: string): string {
  return /^[A-Za-z0-9_-]+$/u.test(key) ? key : quoteString(key);
}
 ...
 */
export function buildTomlTable(header: string, values: Record<string, TomlTableValue>): string {
