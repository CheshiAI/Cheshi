...
class ClaudeCodeTarget implements AgentTarget {
  ...
  uninstall(loc: Location): WriteResult {
    ...
    // 1. MCP server entry
    const mcpPath = mcpJsonPath(loc);
    files.push({ path: mcpPath, action: removeMcpServerEntry(mcpPath) });
    ...
    // uninstall — and the Bun `preuninstall` lifecycle hook that drives it — fully
...
export function writeMcpEntry(loc: Location): WriteResult['files'][number] {
  ...
  return { path: file, action: writeMcpServerEntry(file, getMcpServerConfig()) };
}
 ...
 * `install` (a refresh self-heals) and `uninstall`.
 */
