...
export function getMcpServerConfig(): McpServerConfig {
  ...
  const invocation = codeGraphCliInvocation(['serve', '--mcp']);
  const config: McpServerConfig = {
    type: 'stdio',
    command: invocation.command,
    args: invocation.args,
  };
  if (dataRoot) config.env = { [CODEGRAPH_DATA_ROOT_ENV]: dataRoot };
  return config;
}
...
export function readJsonFile(filePath: string): Record<string, any> {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Could not back up unparseable JSON config ${filePath}; refusing to overwrite it: ${reason}`,
        { cause: backupError },
      );
    }
    return {};
...
export function detectMcpJson(filePath: string, installed: boolean): {
  installed: boolean;
...
export type JsonWriteAction = 'created' | 'updated' | 'unchanged';

export function writeMcpServerEntry(filePath: string, entry: unknown): JsonWriteAction {
  const existing = readJsonFile(filePath);
  const before = existing.mcpServers?.codegraph;
  if (jsonDeepEqual(before, entry)) return 'unchanged';
  const action: JsonWriteAction = before || fs.existsSync(filePath) ? 'updated' : 'created';
  existing.mcpServers ??= {};
  existing.mcpServers.codegraph = entry;
  writeJsonFile(filePath, existing);
  return action;
}

export function removeMcpServerEntry(filePath: string): 'removed' | 'not-found' {
  if (!fs.existsSync(filePath)) return 'not-found';
  const config = readJsonFile(filePath);
  if (!config.mcpServers?.codegraph) return 'not-found';
