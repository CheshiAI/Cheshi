...
/** Bun executes the sibling TypeScript worker directly. */
const WORKER_FILE = codeGraphRuntimePath('workers', 'query-worker.js')
  ?? path.join(__dirname, 'query-worker.ts');
...
function busyGuidance(waitedMs: number): ReadToolResult {
...
export class QueryPool {
  ...
  private settle(job: Job, result: ReadToolResult): void {
  ...
  run(toolName: string, args: Record<string, unknown>): Promise<ReadToolResult> {
    return new Promise<ReadToolResult>((resolve) => {
      const job: Job = {
