...
import { codeGraphCliInvocation } from '../bin/cli-invocation';
...
import { HOST_PPID_ENV, supervisionLostReason, parsePpidPollMs, parseHostPpid } from './ppid-watchdog';
 ...
 * `.codegraph/daemon.log`). Re-invokes the *same* CLI while preserving the
 * public launcher's command prefix (`cheshi-cli codegraph`) and omitting Bun's
 * virtual `/$bunfs/` entry path when the launcher is a compiled executable.
 * The spawned process self-arbitrates the O_EXCL lock, so racing launchers may
 * each spawn one — losers exit and every launcher proxies through the single
 * winner.
 */
function daemonProcessArguments(scriptPath: string, root: string): string[] {
  const embeddedBunEntry = /(^|[\\/])(?:\$bunfs|~bun)(?:[\\/]|$)/iu.test(scriptPath);
  const invocation = codeGraphCliInvocation(['serve', '--mcp', '--path', root]);
  return [
    ...process.execArgv,
    ...(embeddedBunEntry ? [] : [scriptPath]),
    ...invocation.args,
  ];
}
...
function spawnDetachedDaemon(root: string): void {
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      daemonProcessArguments(scriptPath, root),
      {
        detached: true,
        stdio,
        windowsHide: true,
