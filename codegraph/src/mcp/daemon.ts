...
export class Daemon {
  ...
  /** Accepted sockets that have not finished the optional client-hello yet. */
  ...
  async stop(reason: string = 'stop'): Promise<void> {
    ...
    this.clients.clear();
    // `server.close()` waits for accepted connections. A client can disconnect
    // while its optional hello is still being read, before an MCPSession exists;
    // close those sockets explicitly so shutdown cannot wait forever.
  ...
  private handleConnection(socket: net.Socket): void {
    // A connection exists before its optional client-hello has been parsed.
    // Count that handshake window for lifecycle purposes so the zero-client
    // idle timer cannot race server.close() against an untracked open socket.
    ...
    // timeout or a non-hello first line yields null pids and falls back to the
    // socket-close lifecycle. An early close yields `null`, because registering
    // a session after the socket's close event has already fired would leave a
    // phantom client that can never decrement the daemon refcount (#692).
    void readClientHello(socket).then((peers) => {
      this.pendingSockets.delete(socket);
      if (this.stopping || peers === null || socket.destroyed || socket.readableEnded) {
        if (!socket.destroyed) socket.destroy();
        if (!this.stopping && this.clients.size === 0 && this.pendingSockets.size === 0) {
          this.armIdleTimer();
        }
        return;
      }
  ...
  private dropClient(session: MCPSession): void {
    ...
    if (this.clients.size === 0 && this.pendingSockets.size === 0) this.armIdleTimer();
  }

  private armIdleTimer(): void {
    if (this.idleTimer || this.stopping) return;
    if (this.clients.size > 0 || this.pendingSockets.size > 0) return;
    ...
    this.idleTimer = setTimeout(() => {
      ...
      if (this.clients.size > 0 || this.pendingSockets.size > 0) {
        this.armIdleTimer();
        return;
      }
...
export function peerIsDead(
  ...
  return peers.hostPid !== null && !isAlive(peers.hostPid);
}
