 ...
 * loop on the main thread — a runtime pathology, a runaway
 ...
 * obvious first choice and it works in a toy process — but it failed in the
 * real daemon (#850 live test). A child process shares no JavaScript heap with
 * the parent, so a main-thread wedge cannot touch it; it kills via the kernel,
 * which honours
...
/**
 * The watchdog child body, run via `bun -e`. It is inlined so there is no
 * sibling-module path to resolve. Reads its
...
export function installMainThreadWatchdog(options: WatchdogOptions = {}): WatchdogHandle | null {
  ...
  try {
    const runtimeWatchdog = codeGraphRuntimePath(
      `codegraph-watchdog${process.platform === 'win32' ? '.exe' : ''}`,
    );
    const watchdogArgs = [
      String(process.pid),
      String(timeoutMs),
    ...
    const executable = runtimeWatchdog && fs.existsSync(runtimeWatchdog)
      ? runtimeWatchdog
      : process.execPath;
    const args = executable === runtimeWatchdog
      ? watchdogArgs
      : ['-e', CHILD_SOURCE, ...watchdogArgs];
    // No execArgv inheritance (unlike Worker). The child runs no WASM. stderr inherits the parent's
