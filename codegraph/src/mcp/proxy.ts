...
import { HOST_PPID_ENV, supervisionLostReason } from './ppid-watchdog';
 ...
 * socket-close lifecycle). `hostPid` mirrors the PPID watchdog: an explicitly
 * supplied host pid if set, otherwise our own parent.
 */
...
function readHelloLine(socket: net.Socket): Promise<DaemonHello> {
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk: string | Buffer) => {
      ...
      cleanup();
      if (tail.length > 0) {
        // Push back via unshift — Bun's compatible net.Socket supports it.
        socket.unshift(tail);
      }
      try {
