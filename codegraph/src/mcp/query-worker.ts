 ...
 * `bun:sqlite`. `codegraph_explore` is CPU-heavy (FTS + RWR/personalized-
 ...
 * Each worker owns its OWN read connection (bun:sqlite WAL allows N concurrent
 ...
 * dispatch + error classification — and returns the raw {@link ReadToolResult}; the
 * MAIN thread keeps the catch-up gate, watcher-state notices (staleness /
 * worktree), and `codegraph_status`, none of which a watcher-less read
 * connection can answer.
 */

import { parentPort, workerData } from 'worker_threads';
import type { ReadToolResult } from './tools';
...
if (parentPort) {
  ...
  const serve = async (msg: CallMessage): Promise<void> => {
    ...
    try {
      ...
      // into a ReadToolResult and never throws — the catch is belt-and-suspenders.
      const result: ReadToolResult = await handler.executeReadTool(msg.toolName, msg.args);
...
function errorResult(text: string): ReadToolResult {
