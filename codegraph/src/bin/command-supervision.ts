 ...
 *
 *   1. **Orphaned worker.** If an embedding launcher dies without forwarding
 *      a signal, the PPID watchdog (#277) notices the parent/host went away and
 *      exits the child instead of leaving it pinned to a core.
...
export function installCommandSupervision(label: string, watchdog: WatchdogOptions = {}): CommandSupervision {
  ...
  // PPID watchdog: detect that the parent (or an explicitly supplied host)
  // died and we've been orphaned, then exit instead of leaking.
