...
/// Required fields for a node row. Named fields make source positions and
/// string-arena references explicit at construction sites.
...
impl NodeRow {
    pub fn new(input: NodeRowInput) -> Self {
        Self {
            kind: input.kind,
...
impl EdgeRow {
    pub fn new(source_idx: u32, target_idx: u32, kind: u8, metadata_json: StrRef) -> Self {
        Self {
            source_idx,
            target_idx,
            kind,
...
impl RefRow {
    pub fn new(from_idx: u32, kind: u8, line: u32, column: u32, reference_name: StrRef) -> Self {
        Self {
            from_idx,
            kind,
            line,
