...
//! parity — verified by `bun run kernel:parity` and the §5 gate.
...
thread_local! {
    /// Per-call feature state. Extraction is synchronous and may run on
    /// multiple JS worker threads, so thread-local state keeps concurrent N-API
    /// calls independent without threading this flag through every walker.
    static VALUE_REFS_ENABLED: Cell<bool> = const { Cell::new(true) };
}

pub(crate) fn value_refs_enabled() -> bool {
    VALUE_REFS_ENABLED.get()
}

struct RestoreValueRefs(bool);

impl Drop for RestoreValueRefs {
    fn drop(&mut self) {
        VALUE_REFS_ENABLED.set(self.0);
    }
}

fn with_value_refs_enabled<T>(enabled: bool, extract: impl FnOnce() -> T) -> T {
    let previous = VALUE_REFS_ENABLED.replace(enabled);
    let _restore = RestoreValueRefs(previous);
    extract()
}
...
pub fn extract_file(
    ...
    content: String,
    language: String,
) -> Result<ExtractBuffers> {
    let value_refs_enabled = std::env::var("CODEGRAPH_VALUE_REFS").as_deref() != Ok("0");
    extract_file_configured(file_path, content, language, value_refs_enabled)
}

/// Extraction entry point used by current TS callers. Runtime feature state is
/// passed explicitly because Bun can update its JS process.env view without an
/// already-loaded native addon observing the same update through std::env.
#[napi]
pub fn extract_file_configured(
    file_path: String,
    content: String,
    language: String,
    value_refs_enabled: bool,
) -> Result<ExtractBuffers> {
    with_value_refs_enabled(value_refs_enabled, || {
        extract_file_inner(file_path, content, language)
    })
}
...
fn extract_file_inner(
    ...
    let out = match language.as_str() {
        ...
        "c" | "cpp" => {
            ccpp::extract(&file_path, &content, &language).map_err(Error::from_reason)?
        }
        ...
        "lua" | "luau" => {
            lua::extract(&file_path, &content, &language).map_err(Error::from_reason)?
        }
