...
import * as investigationContracts from '../shared/autopilot-investigation';
...
import { SearchClearButton } from '../frontend/src/shared/ui/SearchClearButton';

interface Slot { value?: unknown; dependencies?: readonly unknown[]; cleanup?: () => void }
interface Props { children?: ReactNode; primary?: ReactNode; secondary?: ReactNode; trailingAction?: ReactNode; disabled?: boolean;
...
function elements(node: ReactNode): ReactElement<Props>[] {
  ...
  return [node, ...elements(node.props.children), ...elements(node.props.primary), ...elements(node.props.secondary), ...elements(node.props.trailingAction)];
}
...
function harness(options: { get?: Promise<AutopilotState>; start?: Promise<AutopilotState>; configured?: boolean; available?: boolean; saved?: boolean } = {}) {
  ...
  const modules: Record<string, unknown> = {
    ...
    'lucide-react': { Link: 'svg', Navigation: 'svg', PanelRight: 'svg', Play: 'svg', Search: 'svg', Square: 'svg', Target: 'svg' },
    '../../../../shared/autopilot': contracts,
    '../../../../shared/autopilot-investigation': investigationContracts,
    ...
    '../../shared/ui': { EmptyState, NeumorphicButton: Button, NeumorphicTextField, SearchClearButton, LiquidGlassPanel: 'aside',
  ...
  return { requests, exportsRequested, focusedInputs, resultsComponent: resultExports.AutopilotResearchResults,
    render(active = true, blocked = false) {
      ...
      for (const node of elements(tree)) if (node.props.ref) {
        node.props.ref.current = { focus: () => focusedInputs.push(node.props['aria-label'] ?? '') };
      }
...
function enterInput(h: ReturnType<typeof harness>, label: string, value: string) {
  elements(h.render()).find(node => node.props['aria-label'] === label)!.props.onChange?.({ target: { value } });
}

function enterNavigationGoal(h: ReturnType<typeof harness>) {
  enterInput(h, 'Start URL', 'https://en.wikipedia.org/wiki/DNA');
  enterInput(h, 'Goal', 'Reach the Wikipedia page for Manipuri pony.');
}

test('inputs start empty and switching modes preserves blank or user-entered values', async () => {
  const h = harness();
  try {
    h.render(); await flush();
    const labels = ['Start URL', 'Goal', 'Search text'];
    for (const mode of ['Navigate', 'Research', 'Navigate']) {
      button(h.render(), mode).props.onClick?.();
      for (const label of labels) {
        const input = elements(h.render()).find(node => node.props['aria-label'] === label)!;
        expect(input.props.value).toBe('');
        expect(input.props.placeholder).toBeTruthy();
      }
      expect(button(h.render(), 'Start').props.disabled).toBe(true);
      expect(elements(h.render()).some(node => node.type === SearchClearButton)).toBe(false);
    }
    enterNavigationGoal(h);
    enterInput(h, 'Search text', 'my own search');
    const before = labels.map(label => elements(h.render()).find(node => node.props['aria-label'] === label)!.props.value);
    button(h.render(), 'Research').props.onClick?.();
    expect(labels.map(label => elements(h.render()).find(node => node.props['aria-label'] === label)!.props.value)).toEqual(before);
    expect(button(h.render(), 'Start').props.disabled).toBe(false);
  } finally { h.close(); }
});
...
test('clear actions empty only their input, restore focus and preserve required-field validation', async () => {
  const h = harness();
  try {
    h.render(); await flush();
    enterNavigationGoal(h);
    const input = (label: string) => elements(h.render()).find(node => node.props['aria-label'] === label)!;
    expect(input('Start URL').props.placeholder).toContain('Start URL');
    expect(input('Goal').props.placeholder).toContain('Goal');
    expect(input('Search text').props.placeholder).toContain('Search text (optional)');
    input('Search text').props.onChange?.({ target: { value: 'pony' } });
    for (const [clear, label] of [['Clear search text', 'Search text'], ['Clear start URL', 'Start URL'], ['Clear goal', 'Goal']]) {
      const before = new Map(['Start URL', 'Goal', 'Search text'].map(name => [name, input(name).props.value]));
      elements(h.render()).find(node => node.props['aria-label'] === clear)!.props.onClick?.();
      expect(input(label!).props.value).toBe('');
      expect(h.focusedInputs.at(-1)).toBe(label);
      expect(elements(h.render()).some(node => node.props['aria-label'] === clear)).toBe(false);
      for (const [name, value] of before) if (name !== label) expect(input(name).props.value).toBe(value);
      expect(button(h.render(), 'Start').props.disabled).toBe(label !== 'Search text');
    }
    expect(h.requests).toEqual([]);
  } finally { h.close(); }
});

test('running and pending requests disable input clear actions', async () => {
  const start = createDeferred<AutopilotState>();
  const h = harness({ start: start.promise });
  try {
    h.render(); await flush();
    enterNavigationGoal(h);
    elements(h.render()).find(node => node.props['aria-label'] === 'Search text')!.props.onChange?.({ target: { value: 'pony' } });
    elements(h.render()).find(node => node.type === 'form')!.props.onSubmit?.({ preventDefault() {} });
    const assertDisabled = () => {
      for (const label of ['Clear start URL', 'Clear goal', 'Clear search text']) {
        expect(elements(h.render()).find(node => node.props['aria-label'] === label)!.props.disabled).toBe(true);
      }
    };
    assertDisabled();
    start.resolve({ ...idle, phase: 'thinking' });
    await flush();
    assertDisabled();
  } finally { h.close(); }
});
...
test('missing keys disable Start, and switching pages or opening a modal hides the native viewport', async () => {
  ...
  try {
    ...
    enterNavigationGoal(h);
    expect(button(h.render(), 'Start').props.disabled).toBe(true);
    expect(renderToStaticMarkup(h.render())).toContain('Settings');
    h.render(true, true);
...
test('Research starts with a bounded source target and disables mode changes while collecting', async () => {
  ...
  try {
    h.render(); await flush();
    button(h.render(), 'Research').props.onClick?.();
    enterInput(h, 'Start URL', 'https://www.google.com/');
    enterInput(h, 'Goal', 'Find primary sources about TypeSafe AI and the Jev model.');
    enterInput(h, 'Search text', 'TypeSafe AI Jev');
...
test('research displays question findings, limitations and links to their original evidence', async () => {
  const h = harness();
  try {
    h.render(); await flush();
    h.emit({ ...idle, mode: 'research', phase: 'partial', targetSources: 10, issues: [], sources: [{
      id: 's1', publisher: 'official', section: 'API response schema', sectionId: 'section_2', url: 'https://example.org/', title: 'Original source', accessedAt: '2026-09-17T00:00:00.000Z',
      evidence: 'A concrete claim made by the publisher.', confidence: 1,
    }], investigation: { model: 'selected-model', activeQuestionId: null, plan: { officialDomains: ['example.org'], questions: [{
      id: 'q1', question: 'Is the claim verified?', query: 'claim', externalQuery: 'claim independent test', requireIndependent: true,
    }] }, assessments: [{ questionId: 'q1', sourceId: 's1', role: 'official', relation: 'supports', sufficient: true }],
      report: { answers: [{ questionId: 'q1', status: 'partial', answer: 'The publisher claims this.', sourceIds: ['s1'], comparison: 'Only official evidence.', limitations: 'Independent validation is missing.' }] } } });
    const html = renderToStaticMarkup(h.render());
    expect(html).toContain('The publisher claims this.');
    expect(html).toContain('Independent validation is missing.');
    expect(html).toContain('href="#research-source-s1"');
    expect(html).toContain('id="research-source-s1"');
    expect(html).toContain('selected-model');
    expect(html).toContain('API response schema');
    expect(html).not.toContain('Questions answered');
    h.emit({ ...idle, phase: 'planning' });
    expect(button(h.render(), 'Stop').props.disabled).toBe(false);
  } finally { h.close(); }
});

test('Research displays document reads separately from navigation after more than 20 records', async () => {
  const h = harness();
  try {
    h.render(); await flush();
    h.emit({ ...idle, mode: 'research', phase: 'reading', sources: [], issues: [], targetSources: 10,
      steps: Array.from({ length: 26 }, (_, index) => ({ url: 'https://example.org/', title: 'Document',
        loadMs: 0, decisionMs: 0, confidence: null, kind: index < 6 ? 'navigation' : 'reading' })),
    });
    const html = renderToStaticMarkup(h.render());
    expect(html).toContain('5 / 20 navigation');
    expect(html).toContain('20 / 40 document reads');
    expect(html).not.toContain('25 / 20');
    expect(button(h.render(), 'Stop').props.disabled).toBe(false);
  } finally { h.close(); }
});
