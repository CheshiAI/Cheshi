...
import { APPLE_NOTES_MAX_TITLE_LENGTH, APPLE_NOTES_MAX_BODY_LENGTH } from '../../../../shared/apple-notes';
...
import { retainedNoteDraft, protectNoteDraftsOnClose } from './appleNotesDraft';
import { noteEditorHtml } from './appleNotesEditorContent';
...
function NoteEditorHeader({ note, children }: { note: AppleNote; children: ReactNode }) {
  const timestamp = noteTimestamp(note);
  return <TwoTierHeader className={styles.header} primary={<>
    {timestamp ? <time className={styles.timestamp} dateTime={timestamp.dateTime} title={timestamp.label}>{timestamp.label}</time>
      : <span className={styles.timestamp}>Date unavailable</span>}
    <div className={styles.headerActions}>{children}</div>
  </>} />;
}

function LoadedNoteEditor({ api, document, children, disabled, onSaved, onBusyChange }: Props & { document: AppleNoteDocument }) {
  const [draft] = useState(() => retainedNoteDraft(document, noteEditorHtml(document)));
  ...
  const editor = useEditor({
    ...
    editable: !reason && !disabled && !state.saving,
    editorProps: {
      attributes: { class: styles.body!, 'aria-label': '메모 본문', role: 'textbox', 'aria-multiline': 'true' },
    ...
    onUpdate: ({ editor }) => { draft.edit(draft.getSnapshot().title, editor.getHTML()); },
  });
  editorRef.current = editor;
  const tooLarge = state.html.length + state.title.length * 6 + 10 > APPLE_NOTES_MAX_BODY_LENGTH;
  const saveDisabled = disabled || !!reason || state.saving || state.blocked || !state.dirty || !state.title.trim() || tooLarge;
  saveRef.current = () => {
  ...
  useEffect(() => { editor?.setEditable(!reason && !disabled && !state.saving, false); }, [editor, reason, disabled, state.saving]);
  ...
  return <div className={styles.editor} onKeyDown={event => {
    ...
    <NoteEditorHeader note={state.original}>
      <span className={styles.status} role="status">{state.saving ? 'Saving…' : state.blocked ? 'Review original' : state.dirty ? 'Edited' : state.saved ? 'Saved' : reason ? 'Read only' : ''}</span>
      <NeumorphicButton raised size="icon" aria-label="Discard changes" title="Discard changes" disabled={!state.dirty || state.saving || disabled} onClick={() => {
    ...
    {!reason && <div className={styles.toolbar} role="toolbar" aria-label="메모 서식">
      {toolbar.map(({ label, icon: Icon, run }) => <NeumorphicButton key={label} size="icon" aria-label={label} title={label}
        disabled={disabled || state.saving || !editor} onMouseDown={event => event.preventDefault()} onClick={run}><Icon aria-hidden="true" /></NeumorphicButton>)}
    </div>}
    {(reason || state.error || checkError || tooLarge) && <div className={styles.notice}>
      ...
      {state.blocked && <NeumorphicButton disabled={checking || state.saving} onClick={() => void checkOriginal()}>{checking ? '확인 중…' : '최신 원본 확인'}</NeumorphicButton>}
    </div>}
    <div className={styles.scroll}>
      <article className={styles.page}>
        {reason ? <><h2>{state.original.title}</h2><pre>{state.original.plaintext}</pre></> : <>
          <input type="text" className={styles.title} aria-label="메모 제목" value={state.title} maxLength={APPLE_NOTES_MAX_TITLE_LENGTH}
            disabled={disabled || state.saving} onChange={event => draft.edit(event.target.value.replace(/[\r\n]/g, ''), state.html)} />
    ...
    {latest && <Modal title="최신 Apple 메모 원본" onClose={() => setLatest(null)}>
      ...
      <h3>{latest.title}</h3><pre className={styles.latest}>{latest.plaintext}</pre>
