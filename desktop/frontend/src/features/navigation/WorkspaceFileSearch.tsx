...
import { Modal, NeumorphicTextField, SearchClearButton } from '../../shared/ui';
...
export function WorkspaceFileSearch({ onOpenFile, onClose }: { onOpenFile: (path: string) => void; onClose: () => void }) {
  ...
  return <Modal title="FIND FILE" headerVariant="section" titleIcon={<Search aria-hidden="true" />} className={styles.dialog}
    ...
    <div className={styles.content} onKeyDown={event => handleFileSearchKeyDown(event, {
      ...
      <NeumorphicTextField ref={inputRef} variant="standard" autoFocus type="search" role="combobox" aria-label="Find a workspace file"
        ...
        trailingAction={state.query ? <SearchClearButton variant="ghost" aria-label="Clear file search"
          onKeyDown={event => { if (event.key === 'Enter') event.stopPropagation(); }}
          onClick={() => {
            controller.current?.changeQuery('');
      ...
      <ul ref={list} id={listId} role="listbox" aria-label="Matching files" aria-busy={state.loading} className={styles.results}>
        {state.result.files.map((file, index) => <li key={file.path} id={`${listId}-${index}`} role="option"
          aria-selected={index === state.selectedIndex} className={styles.result}
