...
export function showcaseViewportRequest(element: HTMLElement, page: ShowcasePage): ShowcaseViewRequest {
  ...
  return { page, bounds: { x, y, width, height }, ...(backgroundColor === null ? {} : { backgroundColor }),
...
export function observeShowcaseViewport(element: HTMLElement, api: ShowcaseApi, page: ShowcasePage,
  ...
  let previous = '';
  let lastRequest: ShowcaseViewRequest = { page, visible: false, bounds: { x: 0, y: 0, width: 0, height: 0 } };
  const update = (): void => {
    if (disposed) return;
    const request = showcaseViewportRequest(element, page);
    const key = JSON.stringify(request);
    if (key === previous) return;
    previous = key;
    lastRequest = request;
    const id = ++requestId;
    void api.setView(request).catch(error => {
  ...
  return () => {
    ...
    void api.setView({ ...lastRequest, visible: false }).catch(() => {});
  };
}
