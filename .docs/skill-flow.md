...
스킬은 명시적으로 실행할 때 제공된 판단 입력을 TypeSafe의 Jev로 전송합니다.
Jev를 사용할 수 없으면 같은 판단 입력을 Codex 로그인으로 OpenAI의 Luna low에 보냅니다.
설정의 이전 대화 검색 허용 스위치는 대화 검색 도구에만 적용되며, 이 CLI 실행을 제어하지 않습니다.
Jev는 TypeSafe 이용량을, Luna는 Codex 계정을 사용합니다.
